...
export class UsersUseCasesFactory {
    ...
    static getAndCreateUserDocumentUseCase = ({ config }) =>
        new GetAndCreateUserDocumentUseCase({
            service: UsersServicesFactory.getAndCreateUserDocumentService({
                config,
            }),
            userOperationRequest: UsersRequestsFactory.userOperationRequest,
        });
    ...
    static setUserMasterPasswordUseCase = ({ config }) =>
        new SetUserMasterPasswordUseCase({
            service: UsersServicesFactory.setUserMasterPasswordService({
                config,
            }),
            userOperationRequest: UsersRequestsFactory.userOperationRequest,
        });
    ...
    static subscribeToAuthStateChangeUseCase = ({ config }) =>
        new SubscribeToAuthStateChangeUseCase({
            service: UsersServicesFactory.subscribeToAuthStateChangeService({
                config,
            }),
            userSubscriptionRequest:
                UsersRequestsFactory.userSubscriptionRequest,
        });
}
