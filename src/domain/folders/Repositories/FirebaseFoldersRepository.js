...
export default class FirebaseFoldersRepository extends FoldersRepository {
    constructor({ config, firebaseUtils }) {
        super({});
        this._config = config;
    ...
    async deleteFolderRelatedPasswords({ folderReferenceRequest }) {
        ...
        for (let i = 0; i < MAX_ITERATIONS; i += 1) {
            ...
            const q = query(
                collection(db, 'folders', folderId, 'passwords'),
                limit(DOCS_PER_ITERATION)
            );
            ...
            if (querySnapshot.docs.length < DOCS_PER_ITERATION) break;
    ...
    async subscribeToFolders({ folderSubscriptionRequest }) {
        ...
        const subscription = onSnapshot(q, (snapshot) => {
            const folders = snapshot.docs
                ...
                .map(this._parseSharedWithParam);
    ...
    async shareFolder({ userPublicDetails, folderShareRequest }) {
        ...
        await setDoc(sharedFolderRef, sharedFolder);
